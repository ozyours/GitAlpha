 ...
 * (the default button — e.g. OK — uses the CONFIRM variant: accent/primary
 * background with palette text color) and a text field matching the app's
 * text-input skin for input dialogs. Colors are inlined because the skin is
 * attached to the pane node, not the scene, so the scene's
 * {@code -gitalpha-*} lookups would not resolve (same convention as the
 * button/text-field skins).
 * <p>
 ...
 * text, default button border, default hover, text-field background,
 * text-field border, text-field text, text-field prompt, text-field
 * selection (active highlight), text-field focus border.
 */
public final class DialogSkin extends ThemeSkin
{
	...
	/**
	 * Resolve the twenty-one placeholder colors: pane/header/content colors,
	 * the six NORMAL button colors, the four CONFIRM default-button colors
	 * (background, text, border, hover), and the text-field colors.
	 *
	 * @param _Palette the palette to read colors from
	 * @return the twenty-one placeholder values
	 */
	@Override
	protected Object[] GetColorArguments(ColorPalette _Palette)
	{
		...
		// argument, so the button color arrays are unfolded here instead
		...
		// Default-button colors sourced from CONFIRM (primary bg/text/border +
		// derive hover) instead of hardcoded Primary + white — keeps the
		// default button consistent with the CONFIRM variant across palettes.
		String[] __ConfirmColors = EButtonVariant.CONFIRM.ResolveSkinColors(_Palette);
		return new Object[] {
				...
				__ConfirmColors[0], __ConfirmColors[1], __ConfirmColors[2], __ConfirmColors[3],
				...
				_Palette.GetActiveHighlightColor().GetHex(__Lookup), __ConfirmColors[0] };
	}
}