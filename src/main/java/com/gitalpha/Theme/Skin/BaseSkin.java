 ...
 * the combo-box popup skin and the context-menu skin (composed from
 * {@link ContextMenuSkin#GetSceneCss}, the single source of its CSS).
 * Popups render in their own scenes that node-level skins cannot reach, but
 * inherit this scene stylesheet, so their rules resolve the scene's
 * variables — the only way to theme them. Consequently {@link #Bake} is
 * overridden instead of filling placeholders, and the abstract hooks return
 * the focus ring fragment / no arguments for consistency.
 */
public final class BaseSkin extends ThemeSkin
{
	 ...
	 * context-menu skin (via {@link ContextMenuSkin#GetSceneCss}).
	 *
	...
	public String Bake(ColorPalette _Palette)
	{
		String __Css = CSS_FORMAT + "\n" + _Palette.GetCssOverrides()
				+ COMBO_BOX_POPUP_CSS_FORMAT + ContextMenuSkin.GetSceneCss();
